...
exports.run = async(client, message, args) => {
  ...
  if (!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send(`:x: Bunun İçin Yetkin Yok`);
  let guild = message.guild
  let reason = args.slice(1).join(' ');
  let user = message.mentions.users.first();
  if (!user) return message.reply('Sunucudan atılacak kişiyi belirtmedin');
  if (!reason) return message.reply('Sunucudan atma sebebini yazmalısın.').catch(console.error);
